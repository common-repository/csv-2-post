...
abstract class CSV2POST_View {
    ...
    /**
     * Initialize the View class, by setting the correct screen columns and 
     * adding help texts.
     *
     * @since 8.1.3
     * 
    ...
    public function __construct() {
        ...
        $this->TABMENU = $this->CONFIG->load_class( 'CSV2POST_TabMenu', 'class-pluginmenu.php', 'classes' );
        $this->Help = $this->CONFIG->load_class( 'CSV2POST_Help', 'class-help.php', 'classes' );
        ...
        // TODO 3 -o Ryan Bayne -c Depreciated: Replace $this->DB with trait class.
        $this->DB = $this->CONFIG->load_class( 'CSV2POST_DB', 'class-wpdb.php', 'classes' );
    ...
    */
    public function setup( $action, array $data ) {
        ...
        // TODO 5 -o Ryan Bayne -c Core: By WP 4.6 we will be able to enqueue on a per view basis
        // without adding this code to the view itself. So move all queuing to one place.
        $CSV2POST_Class->enqueue_script( 'common', array( 'jquery', 'postbox' ), array() );
    ...
    */
    public function dashboard_widgets( $meta_box_array ) { 
        ...
        $this->TABMENU = CSV2POST::load_class( 'CSV2POST_TabMenu', 'class-pluginmenu.php', 'classes' );
    ...
    */
    protected function action_nonce_field( array $data, array $box ) {
        wp_nonce_field( $this->UI->nonce( $this->action ) ); echo "\n";
    }
    ...
    */
    ...
    /**
    * Render the current view of meta boxes, including optional tab menu 
    ...
    */       
    public function render() { 
        global $c2p_tab_number, $CSV2POST_Class;

        // get the admin page name (slug in menu array,
        //without prepend "csv2post_")
        $admin_page = $this->UI->get_admin_page_name();
        
        // if we are on the main page change $admin_page to 'main' 
        //as that is what we use in array
        ...
        $CSV2POST_Class->pageheader( $this->menu_array[ $admin_page ]['title'], 0);
        ...
        if( !isset( $this->menu_array[ $admin_page ]['tabmenu'] ) || $this->menu_array[ $admin_page ]['tabmenu'] === true ) {
            if( $admin_page !== 'main' ) {
                $this->UI->build_tab_menu( $admin_page );
            }
        }
