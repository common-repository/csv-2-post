...
class CSV2POST_UI extends CSV2POST {     
    ...
    */
    public function screens_menuoptions( $current ){
        ...
        foreach( $CSV2POST_Menu as $page_slug => $page_array ){ 
    ...
    /**
    * Use to construct HTML form menu - adds an option for each plugin
    * page.
    * 
    * @author Ryan R. Bayne
    * @package CSV 2 POST
    ...
    */               
    public function page_menuoptions( $current, $return = false ){
        ...
        foreach( $CSV2POST_Menu as $page_slug => $page_array ){ 
            ...
            if( $return){
                return '<option value="'.$page_slug.'" '.$selected.'>'.$CSV2POST_Menu[$page_slug]['title'].'</option>';
            }else{ 
                echo '<option value="'.$page_slug.'" '.$selected.'>'.$CSV2POST_Menu[$page_slug]['title'].'</option>';
            }
        }    
    }
    ...
    */
    public function option_project_headers_checkboxes( $project_id, $name, $validation = false ){
        ...
        $project_columns_array = $this->DB->get_project_columns_from_db( $project_id, true);
    ...
    */
    public function display_current_project() {
        // TODO 1 Task: replace the global use of $CSV2POST a class object
        global $csv2post_settings, $CSV2POST_Class, $wpdb;
        ...
        {
            $current = $CSV2POST_Class->get_project_name( $csv2post_settings['currentproject'] );
        ...
        {
            $source_array = $this->DB->get_project_sourcesid( $projectid );
            ...
            $project_message = '';
            if( !$CSV2POST_Class->does_project_table_exist( $projectid ) )
            {  
            	$tablename = CSV2POST_DBTrait::get_project_main_table( $projectid );
                $project_message = self::notice_return( 
	                'error', 
	                'Tiny', 
	                sprintf( __( 'Project (%s) Table Not Found' ), $projectid ), 
	                sprintf( __( 'Your current active project 
	                no longer has a main database table. The expected table is %s. Either delete the project
	                fully or attempt to restore the table. ' ), $tablename ) 
                );
            }            
            
            $quick_actions .= $project_message;
            
            $quick_actions .= $CSV2POST_Class->linkaction( $_GET['page'], 'createpostscurrentproject', __( 'Create posts using the current project' ), __( 'Create Posts' ), '' ); 
            
            $quick_actions .= $CSV2POST_Class->linkaction( $_GET['page'], 'updatepostscurrentproject', __( 'Update posts originally created by the current project' ), __( 'Update Posts' ), '' );
            
            $quick_actions .= $CSV2POST_Class->linkaction( $_GET['page'], 'importdatacurrentproject', __( 'Import data for the current project' ), __( 'Import Data' ), '' );
            
            $quick_actions .= $CSV2POST_Class->linkaction( $_GET['page'], 'updatedatacurrentproject', __( 'Update the current projects data' ), __( 'Update Data' ), '' ); 
            
            $quick_actions .= $CSV2POST_Class->linkaction( $_GET['page'], 'displayprojectsummary', __( 'Display the current projects summary' ), 'Project ID: ' . $projectid, '' );      
            
            $quick_actions .= $CSV2POST_Class->linkaction( $_GET['page'], 'displaysourcesummary', __( 'Display information about the projects sources' ), 'Source ID: ' . $source_array[0], '' );       
            
            $quick_actions .= $CSV2POST_Class->linkaction( $_GET['page'], 'currentprojectprevious', __( 'Open the previous project and make it the curren active project', 'csv2post' ), __( 'Previous Project', 'csv2post' ), '' );
            
            $quick_actions .= $CSV2POST_Class->linkaction( $_GET['page'], 'currentprojectnext', __( 'Open the next project and make it the current active one', 'csv2post' ), __( 'Next Project', 'csv2post' ), '' );
        

            if( $_GET['page'] == 'csv2post_projectchecklist' )
            {
                $quick_actions .= $CSV2POST_Class->linkaction( 
                	$_GET['page'], 
                	'columnreplacementtokens', 
                	__( 'View a list of the current projects column replacement tokens', 'csv2post' ), 
        ...
        echo self::info_area( 'Current Project (CP): ' . $current, $quick_actions );
    }
    ...
    */
    public function metabox_icons( $form_id, $trash_icon = false ){
        ...
        global $c2pm, $c2p_tab_number;
        
        // load help array
        $CSV2POST_Help = CSV2POST::load_class( 'CSV2POST_Help', 'class-help.php', 'classes' );
        ...
        if( isset( $help_array[ $page_name ][ CSV2POST_VIEWNAME ][ 'forms' ][ $form_id ][ 'formvideoid' ] ) ){
            self::panel_video_icon( $help_array[ $page_name ][ CSV2POST_VIEWNAME ][ 'forms' ][ $form_id ][ 'formvideoid' ] );
            ++$total_icons;        
        } 

        // do we have all required values to build help content and display form_information_icon()
        if( isset( $help_array[ $page_name ][ CSV2POST_VIEWNAME ][ 'forms' ][ $form_id ][ 'formtitle' ] )
            && isset( $help_array[ $page_name ][ CSV2POST_VIEWNAME ][ 'forms' ][ $form_id ][ 'formabout' ] ) ){  
                           
            // display the icon that will show information about the box when clicked
            self::box_information_icon( $page_name, CSV2POST_VIEWNAME, $form_id );
        }
    ...
    public function panel_trash_icon( $form_name ){?>
        <a href="<?php echo $form_name;?>"><img src="<?php echo CSV2POST_IMAGES_URL . 'trash-icon.gif';?>" alt="<?php _e( 'Reset form' );?>" title="<?php _e( 'Reset form' );?>"></a><?php 
    }
    
    public function panel_video_icon( $youtubeid ){
        if( $youtubeid){
            echo '<a href="https://www.youtube.com/watch?v=' . $youtubeid . '" target="_blank"><img src="' . CSV2POST_IMAGES_URL . 'video-icon.gif" alt="' . __( 'View video on YouTube' ) . '" title="' . __( 'View video on YouTube' ) . '"></a>';
        }
    }
    ...
    */
    public function helpicon( $url){
        return '<a href="'.$url.'" title="'. __( 'View help for this on the plugins website.' ) .'" target="_blank"><img src="'. CSV2POST_IMAGES_URL . 'help-icon.png" alt="'. __( 'View help for this on the plugins website' ) .'"></a>';
    }    
    public function videoicon( $url){
        return '<a href="'.$url.'" title="'. __( 'View a video about this feature.' ) .'" target="_blank"><img src="'. CSV2POST_IMAGES_URL . 'video-icon.png" alt="'. __( 'Video icon, click to open a video' ) .'"></a>';
    }     
    public function trashicon( $url){
        return '<a href="'.$url.'" title="'. __( 'Delete or hide the current feature.' ) .'" target="_blank"><img src="'. CSV2POST_IMAGES_URL . 'trash-icon.png" alt="'. __( 'Trash icon, click to delete or hide the current item' ) .'"></a>';
    }    
    public function discussicon( $url){
        return '<a href="'.$url.'" title="'. __( 'Discuss this feature on the WebTechGlobal forum.' ) .'" target="_blank"><img src="'. CSV2POST_IMAGES_URL . 'chat-icon.png" alt="'. __( 'Chat icon which you can click to discuss this feature on the WebTechGlobal forum.' ) .'"></a>';
    }  
    ...
    */          
    public function box_information_icon( $page_name, $view_name, $form_id ) {
        ...
        $all_help_content .= '<img src="' . CSV2POST_IMAGES_URL . 'info-icon.gif" alt="';
    ...
    public function get_notice_array() {
        ...
        $eci_notice_array = maybe_unserialize( $a);
        if(!is_array( $eci_notice_array ) ){
            return array();    
        }
        
        // delete some expired admin notices
        if( isset( $eci_notice_array['admin'] ) )
        {
            foreach( $eci_notice_array['admin'] as $key => $notice){
                ...
                if( isset( $notice['created'] ) )
                {
                    ...
                    if( $projected < time() )
                    {
                        unset( $eci_notice_array['admin'][$key] );    
                    }                                               
                }
                else
                {       
        ...
        if( isset( $eci_notice_array['users'] ) ){
 
            foreach( $eci_notice_array['users'] as $owner_id => $owners_notices){
                ...
                foreach( $owners_notices as $key => $notice)
                {
                    if( isset( $notice['created'] ) )
                    {
                        ...
                        if( $projected < time() )
                        {
                            unset( $eci_notice_array['users'][$key] );    
                        }                                               
                    }
                    else
                    {
        ...
        return $eci_notice_array;    
    }
    
    public function update_notice_array() {
        global $eci_notice_array;      
        return update_option( 'csv2post_notifications',maybe_serialize( $eci_notice_array ) );    
    }    
    ...
    */
    public function create_notice( $message, $type = 'info', $size = 'Small', $title = false, $sensitive = false, $helpurl = false ){
        global $eci_notice_array, $current_user;
                
        // TODO 1 Task: log all notices here, will allow functions doubling as 
        // manual or automated to create more log entries
        ...
        if( $sensitive === false ) {   
            ...
            $eci_notice_array['notices'][$current_user->ID][$key]['sensitive'] = false;
            $eci_notice_array['notices'][$current_user->ID][$key]['message'] = $message;
            $eci_notice_array['notices'][$current_user->ID][$key]['type'] = $type;
            $eci_notice_array['notices'][$current_user->ID][$key]['size'] = $size;
            $eci_notice_array['notices'][$current_user->ID][$key]['title'] = $title; 
            $eci_notice_array['notices'][$current_user->ID][$key]['created'] = time();
            $eci_notice_array['notices'][$current_user->ID][$key]['helpurl'] = $helpurl;
            ...
            $eci_notice_array['users'][$current_user->ID][$key]['sensitive'] = false;
        ...
        return $this->update_notice_array( $eci_notice_array );
    }
    ...
    */
    public function create_prompt() {
        ## $eci_notice_array['prompts'][$owner][$key]['message'] = $message;    
    }
    ...
    */
    public function create_message() {
        ## $eci_notice_array['messages'][$owner][$key]['message'] = $message;    
    }
    ...
    public function display_users_notices() {
        global $eci_notice_array, $current_user, $csv2post_settings;
        
        $eci_notice_array = $this->get_notice_array();
        
        if(!isset( $eci_notice_array['notices'][$current_user->ID] ) ){return;}
 
        foreach( $eci_notice_array['notices'][$current_user->ID] as $key => $owners_notices){
            ...
            unset( $eci_notice_array['notices'][$current_user->ID] );
        }
                                       
        $this->update_notice_array( $eci_notice_array );            
    }
    ...
    */
    ...
    /**
    * New Notice Function (replacing CSV2POST_Notice::notice_depreciated() )
    ...
    */
    public function n_depreciated( $title, $mes, $style, $size, $atts = array() ){
        ...
        // this allows the notice to be printed within content where the function is called rather than within the $eci_notice_array loop
        ...
        $eci_notice_array = $this->persistentnotifications_array();
        
        // set next array key value
        $next_key = 0;

        // determine next array key
        if( isset( $eci_notice_array['notifications'] ) ){    
            $next_key = CSV2POST::get_array_nextkey( $eci_notice_array['notifications'] );
        }    
        ...
        $eci_notice_array['notifications'][$next_key]['message'] = $mes;
        $eci_notice_array['notifications'][$next_key]['type'] = $style;
        $eci_notice_array['notifications'][$next_key]['size'] = $size;
        $eci_notice_array['notifications'][$next_key]['title'] = $title;
        $eci_notice_array['notifications'][$next_key]['helpurl'] = $url; 
        $eci_notice_array['notifications'][$next_key]['output'] = $output;
        $eci_notice_array['notifications'][$next_key]['audience'] = $audience;
        $eci_notice_array['notifications'][$next_key]['user_mes'] = $user_mes;
        $eci_notice_array['notifications'][$next_key]['side'] = $side;
        $eci_notice_array['notifications'][$next_key]['clickable'] = $clickable;
                          
        $this->update_persistentnotifications_array( $eci_notice_array );
    } 
    ...
    */
    public function notice_depreciated( $message, $type = 'success', $size = 'Extra', $title = false, $helpurl = 'csv2post.wordpress.com', $output_type = 'echo', $persistent = false, $clickable = false, $user_type = false ){
        global $eci_notice_array;
        if( is_admin() || $output_type == 'public' ){
            ...
            if( $output_type == 'return' || $output_type == 'public' ){   
                ...
                // add new message to the notifications array
                $eci_notice_array['notifications'][$next_key]['message'] = $message;
                $eci_notice_array['notifications'][$next_key]['type'] = $type;
                $eci_notice_array['notifications'][$next_key]['size'] = $size;
                $eci_notice_array['notifications'][$next_key]['title'] = $title;
                $eci_notice_array['notifications'][$next_key]['helpurl'] = $helpurl; 
                $eci_notice_array['notifications'][$next_key]['clickable'] = $clickable; 
    ...
    /**                                              
    * Outputs the contents of $eci_notice_array, used in CSV2POST::pageheader().
    * Will hold new and none persistent notifications. May also hold persistent. 
    */
    public function output_depreciated() {
        $eci_notice_array = self::persistentnotifications_array();    
        if( isset( $eci_notice_array['notifications'] ) ){
                                                        
            foreach( $eci_notice_array['notifications'] as $key => $notice){
                ...
                if(!isset( $notice['persistent'] ) || $notice['persistent'] != false ){
                    ...
                    unset( $eci_notice_array['notifications'][$key] );
                }
            }
...
class CSV2POST_ProjectDataSources_Table extends WP_List_Table {
...
class CSV2POST_CategoryProjection_Table extends WP_List_Table {
...
class CSV2POST_ReplaceValueRules_Table extends WP_List_Table {
