...
class CSV2POST_Forms extends CSV2POST_UI {
    ...
    */
    public function apply_form_security() {
        ...
        if( !isset( $_POST['csv2post_form_name'] ) )
        {
            return false;
        }
        ...
        $nonce_result = wp_verify_nonce( $_POST['_wpnonce'], $_POST['csv2post_form_name'] );
        if( !$nonce_result )
        {
            return false;
        }
    ...
    */
    public function form_start( $form_id, $form_name, $form_title, $uploader = false ){
        ...
        $form_id = esc_attr( strtolower( $form_id ) );
        ...
        echo '<form method="post" name="' . $form_name . '" id="' . $form_id . '" action="' . $action . '"' . $enctype . '>';
    ...
    public function input_checkboxesgrouped(){
        ...
        <tr valign="top">
            ...
            foreach( $this->itemsarray as $item_value => $item_title ) {
                ...
                // Set checked status using stored settings, $checkedstatus is boolean, it may not exist at all.
                $checked = '';      
                foreach( $this->currentvalue as $item_value_stored => $checkedstatus )
                {
					if( $item_value === $item_value_stored && $checkedstatus == true )
					{
						$checked = ' checked';	
					}	
                }
    ...
    */
	...
	/**
	* jQuery Data and Time input.
	* 
	* @version 1.0
	* 
	* @todo if default date and time is submitted then the value is "Date", the
	* value attribute needs to be initialized better or force the user to interact
	* with the date time picker.
	*/
	public function input_datetimepicker() {
    	$itemID = esc_attr( $this->inputid );
		?>
        ...
        <tr>    
            <td>	
				...
				<input type="text" id="<?php echo $itemID; ?>" name="<?php echo $itemID; ?>" value="Date"/>

				<script type="text/javascript">   
					jQuery(document).ready(function() {
					    jQuery('#<?php echo $itemID; ?>').datetimepicker({
					        inline:true
					    });
					});      
				</script>	
    ...
    */
    public function checkboxes_basic( $formid, $id, $name, $title, $items_array, $current_value, $required = false, $validation_array = array(), $groupcheckboxes = true ) {
        // Avoid Warning: Invalid argument supplied for foreach().
        if( !is_array( $items_array ) ) {
			$items_array = array();
        }        
        self::input( $formid, 'checkboxes', $id, $name, $title, $title, $required, $current_value, array( 'groupcheckboxes' => $groupcheckboxes, 'itemsarray' => $items_array ), $validation_array );        
    } 
    ...
    * @version 1.1
    */
    public function checkboxessingle_basic( $formid, $id, $name, $title, $items_array, $current_value, $required = false, $validation_array = array() ) {
        // Avoid Warning: Invalid argument supplied for foreach().
        if( !is_array( $items_array ) ) {
			$items_array = array();
        }       
        self::input( $formid, 'checkboxessingle', $id, $name, $title, $title, $required, $current_value, array( 'itemsarray' => $items_array ), $validation_array );        
    } 
    ...
    * @version 1.1
    */
    public function checkboxesgrouped_basic( $formid, $id, $name, $title, $items_array, $current_value, $required = false, $validation_array = array() ) {
        // Avoid Warning: Invalid argument supplied for foreach().
        if( !is_array( $items_array ) ) {
			$items_array = array();
        }
        self::input( $formid, 'checkboxesgrouped', $id, $name, $title, $title, $required, $current_value, array( 'itemsarray' => $items_array ), $validation_array );        
    }
    ...
    * @version 1.0
    */
    public function datatimepicker_basic( $formid, $id, $name, $title, $current_value, $required = false, $validation_array = array() ) {  
        self::input( $formid, 'datetimepicker', $id, $name, $title, $title, $required, $current_value, array(), $validation_array );        
    }
