...
//class CSV2POST_PHP extends CSV2POST_Flags { commented 14092014
class CSV2POST_PHP {  
    /**
    * Performs a var_dump if debug_mode active (debug_mode is only active when 
    * on installation blog unless manually activated)
    * 
    * @author Ryan R. Bayne
    * @package CSV 2 POST
    * @version 1.2
    */   
    public function var_dump( $value, $header = false, $append = '<br>' ){
        if( defined( "WEBTECHGLOBAL_DEBUG_DISPLAY" ) && WEBTECHGLOBAL_DEBUG_DISPLAY == true ) {
    ...
    */ 
    ...
    /**
     * Counts rows in CSV file and returns (does no include header row). Using
     * WordPress core function to ensure trailing slash.
     * 
    ...
    public function count_csvfilerows( $route, $file, $include_header = false ){ 
        $counted = count( file( trailingslashit( $route ) . $file ) ); 
    ...
    */
    ...
    /**
    * Compares giving possible/expected prepend (needle) characters to haystack 
    * string and determines if the needle exists at the beginning of the haystack.
    * 
    ...
    */
    ...
    /**
    * Used for testing/debugging. Dumps the giving __LINE__ with a string of 
    * text to help locate the use of the function if not removed. 
    * 
    * 1. Does not print anything if debug_mode not on, this helps to keep the 
    * interface clean if these are left in functions 
    * 
    * @param mixed $line
    * @param mixed $string
    * @version 1.2
