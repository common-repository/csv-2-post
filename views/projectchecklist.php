...
class CSV2POST_Projectchecklist_View extends CSV2POST_View {
    ...
    */
    public function viewintroduction() {
        $main_title = __( 'Project Checklist Introduction', 'csv2post' );
        $intro = __( 'My goal is to not only create an interesting project checklist but a functional one too. I want this checklist to be highly interactive. Right now colors are used to tell us the state of a setting. If something is required it will show in red and needs attention. So this new checklist added June 2015 might become one of the most used features in CSV 2 POST - but I am not done with it. I want this view to double as an easy step by step system for none technical users and will finally be the screen everyone asks for. Clicking on each item will open a form and we can change settings right there. This is a lot of work though and I require more funding from anyone who does not have an empty Paypal account as I do', 'csv2post' );
        ...
        $this->UI->intro_box_dismissible( 'projectchecklist-introduction', $main_title, $intro, $info_area = true, $title, $info, $foot );               
    }
