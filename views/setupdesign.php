 ...
 * @since 8.1.3
 */
class CSV2POST_Setupdesign_View extends CSV2POST_View {
    ...
    protected $view_name = 'setupdesign';
    ...
    */
    public function viewintroduction() {
        $main_title = __( 'Content Designs Explained', 'csv2post' );
        
        $intro = __( 'The key to designing your template that holds the
        layout and styling for post content are my Column Replacement Tokens.
        They are basically .csv file column names/headers wrapped in characters
        to tell the plugin that they should be replaced with their related data.
        Different tokens are available to call on advanced abilities i.e. random
        number generator, the grabbing then insertion of a Tweet and more. A design
        can be very advanced but it can also be simple for simple posts.', 'csv2post' );
        ...
        $this->UI->intro_box_dismissible( 'content-introduction', $main_title, $intro, $info_area = true, $title, $info, $foot );               
    }
    ...
    * @version 1.1
    */
    public function postbox_setupdesign_defaulttitletemplate( $data, $box ) {    
        ...
        ?>  
            ...
            if( isset( $this->current_project_settings['titles']['defaulttitletemplate'] ) ){
                ...
                if( $result ) {
                    ...
                    $projectcolumns = $this->DB->get_project_columns_from_db( $csv2post_settings['currentproject'] );
    ...
    * @version 1.1
    */
    public function postbox_setupdesign_defaultcontenttemplate( $data, $box ) { 
       
        $this->UI->postbox_content_header( $box['title'], $box['args']['formid'], 
        __( 'This is the first editor you should use. If you do not plan to 
        create multiple templates in a more advanced project, this is the 
        only editor you need to use.', 'csv2post' ), false ); 
    ...
    * @version 1.1
    */
    public function postbox_setupdesign_newcontenttemplate( $data, $box ) {    
    ...
    * @version 1.1
    */
    public function postbox_setupdesign_multipledesignsrules( $data, $box ) {    
        ...
        ?>  
            ...
            for( $i=1;$i<=3;$i++){
                ...
                if( isset( $this->current_project_settings['content']["designrule$i"]['table'] ) ){
                    $designrulecolumn_table = $this->current_project_settings['content']["designrule$i"]['table'];
                }
                if( isset( $this->current_project_settings['content']["designrule$i"]['column'] ) ){
                    $designrulecolumn_column = $this->current_project_settings['content']["designrule$i"]['column'];
                }             
    ...
    * @version 1.1
    */
    public function postbox_setupdesign_groupimportlocalimages( $data, $box ) {    
